    ...
    const channel = supabase.channel('admin-changes')
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'profiles' }, (payload) => {
        setUsers(prev => {
          let newUsers = [...prev];
          if (payload.eventType === 'INSERT') {
            if (!newUsers.find(u => u.id === payload.new.id)) newUsers.push(payload.new);
          } else if (payload.eventType === 'DELETE') {
            newUsers = newUsers.filter(u => u.id !== payload.old.id);
          } else if (payload.eventType === 'UPDATE') {
            newUsers = newUsers.map(u => u.id === payload.new.id ? payload.new : u);
          }
          setBannedUsers(newUsers.filter(u => u.is_banned));
          return newUsers;
        });
      })
